...
ret;
	...
	<searchLink>
	  <link>
	    <script>"<?php echo $host."/scripts/clip/php/vimeo.php?query=,"; ?>" + urlEncode(keyword) + "," + urlEncode(keyword);</script>
	  </link>
	</searchLink>
	<searchLink1>
	  <link>
...
<channel>
	...
	<menu>main menu</menu>
		<item>
			<title>Search for a video tag</title>
		...
		<item>
			<title>Display video from a user (ex. 4936621)</title>
...
foreach($videos as $video) {
	...
	$link = $host."/scripts/clip/php/vimeo_can.php?query=,".$cat.",".urlencode($title);
  echo '
  <item>
  <title>'.$title.'</title>
  <link>'.$link.'</link>
