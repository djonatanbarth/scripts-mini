...
<channel>
	<title>www.gaskrank.tv</title>
	<menu>main menu</menu>


<?php
$page = $_GET["query"];
//http://www.gaskrank.tv/tv/motorrad-videos/
//http://www.gaskrank.tv/tv/motorrad-videos/filme-15.htm
//http://www.gaskrank.tv/tv/motorrad-videos/filme-15.html
if ($page == 1) {
   $link="http://www.gaskrank.tv/tv/motorrad-videos/";
} else {
  $page1=15*($page-1);
  $link="http://www.gaskrank.tv/tv/motorrad-videos/filme-".$page1.".htm";
}
...
foreach($videos as $video) {
  ...
  $t2=explode('"',$t1[1]);
  $link="http://www.gaskrank.tv".$t2[0];
  $t1 = explode('src="', $video);
  $t2 = explode('"', $t1[1]);
  $image = "http://www.gaskrank.tv".$t2[0];
  ...
  $data=trim(str_between($video,"</strong></a><br>","<br>"));
  $t1=explode('class="tipp_content">',$video);
  $t2=explode("<br>",$t1[1]);
  //print_r($t2);
  $t3=explode("</div>",$t2[9]);
  $annotation=trim($t3[0]);
  //$annotation=trim(str_between($t2[8],"<br><br>","</div>"));
  $name = preg_replace('/[^A-Za-z0-9_]/','_',$title).".flv";
  $link = 'http://127.0.0.1/cgi-bin/scripts/clip/php/gaskrank_link.php?file='.$link;
	echo'
	<item>
	<title>'.$title.'</title>
    <onClick>
