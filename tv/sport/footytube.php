...
if (userInput == "two" || userInput == "2")
	{
     showIdle();
     url="<?php echo $host; ?>" + "/scripts/tv/sport/footytube_link.php?file=" + getItemInfo(getFocusItemIndex(),"download");
     movie=getUrl(url);
     cancelIdle();
...
$html = file_get_contents("http://www.footytube.com/more.php?type=Most%20Recent%20Matches&page=".$page);
...
$videos = explode('<div class="vid_border">', $html);
...
foreach($videos as $video) {
    ...
    $t2 = explode('"', $t1[1]);
    $link = "http://www.footytube.com".$t2[0];

    $t1 = explode('src="', $video);
    $t2 = explode('"', $t1[1]);
    $image = $t2[0];
    
    $title=str_between($video,'"standard_link">','<');
    ...
    <script>
    showIdle();
    url="'.$host.'/scripts/tv/sport/footytube_link.php?file='.urlencode($link).'";
    movie=getUrl(url);
    titlu=movie;
    cancelIdle();
