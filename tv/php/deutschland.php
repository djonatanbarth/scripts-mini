...
if (userInput == "pagedown" || userInput == "pageup")
{
  ...
  ret="true";
...
else if(userInput == "enter")
      {
        showIdle();
        focus = getFocusItemIndex();
        stream_u = "http://127.0.0.1/cgi-bin/scripts/tv/php/deutschland_link.php?file=" + getItemInfo(focus, "stream_u");
...
<script>
    channelImage = "http://www.surfmusik.de/logo1.gif";
  </script><channel>
  <title>Internet Radio Deutschland</title>
<?php
//<td class="home">
$link="http://www.surfmusik.de/webradio/deutschland.htmlthis";
$html=file_get_contents($link);
$videos = explode('<td class="home">', $html);
unset($videos[0]);
...
foreach($videos as $video) {
  ...
  $link=$t2[0];
  $title=$t4[0];
  $sources=explode('class="navil"',$video);
  unset($sources[0]);
  $sources = array_values($sources);
  foreach($sources as $source) {
    $t1=explode('href="',$source);
    $t2=explode('"',$t1[1]);
    $t3=explode('>',$t1[1]);
    $t4=explode('<',$t3[1]);
    $m=$t4[0];
