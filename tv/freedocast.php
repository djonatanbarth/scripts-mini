...
>
  ...
  image/tv_radio.png
		...
		<text align="center" redraw="yes"
          fontSize=17
		      offsetXPC=55 offsetYPC=65 widthPC=40 heightPC=10
...
foreach($videos as $video) {
    ...
    $t1=explode('<span class="grey12nor">',$video);
    $t2=explode('<',$t1[1]);
    $gen=trim($t2[0]);
    $t3=explode('<',$t1[2]);
    $by=trim($t3[0]);
    ...
    <script>
    showIdle();
    url=geturl("http://127.0.0.1/cgi-bin/scripts/tv/freedocast_link.php?file='.$link.'");
    annotation=url;
    movie="'.$baseurl.'" + url;
    cancelIdle();
