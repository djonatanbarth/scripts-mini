...
$image="http://www.moovie.cc/images/movies/".$id."/poster.jpg";
$adn=trim(str_between($html,'<div id="plot">','</div>'));
...
$videos = explode('class="site" id="site_', $html);
unset($videos[0]);
...
foreach($videos as $video) {		
   $t1=explode('"',$video);
   $id=$t1[0];
   $l="http://www.moovie.cc/links.php?id=".$id;
   $t2=explode(">",$t1[1]);
   $t3=explode("<",$t2[1]);
   $server=$t3[0];
