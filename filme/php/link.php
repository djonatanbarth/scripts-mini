...
} elseif ((strpos($filelink, 'fastupload.rol.ro') !== false)  || (strpos($filelink, 'fastupload.ro') !== false)) {
   $h = file_get_contents($filelink);
   $link=str_between($h,"file': '","'");
...
} elseif (strpos($filelink,"http://nosvideo.com") !== false) {
   $h=file_get_contents($filelink);
...
//echo $filelink;
 ...
 //http://roshare.info/embed-6cm2e35qoj42-600x390.html"
  ...
  if (strpos($l,"embed") === false) {
   $t1=explode("/",$l);
   $id=$t1[3];
   $l="http://roshare.info/embed-".$id."-600x390.html";
  }
  $h=file_get_contents($l);
