...
//http://www.starmaxmovie.com/videos.php?vid=dd4f45f2d
//http://www.starmaxmovie.com/browse-1-videos-1-date.html
//http://www.starmaxmovie.com/browse-1-videos-2-date.html
$link = substr($search,0, -11).$page."-date.html";
...
foreach($videos as $video) {		
  ...
  $t2 = explode('"', $t1[1]);
  $link = $t2[0];
  //http://www.starmaxmovie.com/24/sez1-ep24-video_47bfea05d.html
  $id=substr(substr(strrchr($link, "_"), 1),0,-5);
  $link="http://www.starmaxmovie.com/videos.php?vid=".$id;
  $t1=explode('src="',$video);
  $t2=explode('"',$t1[1]);
  $image=$t2[0];
  $title=str_between($video,'<span class="artist_name">','</a>');
  $title=trim(str_replace("</span>","",$title));
	if ($link <> "") {
		$link1 = $baseurl.$link;
