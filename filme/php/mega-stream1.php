...
$t1=explode('a target="_blank"',$html);
$t2=explode('src="',$t1[1]);
$t3=explode('"',$t2[1]);
$image=$t3[0];
if (strpos($image,"http") === false) {
$image="http://www.mega-stream.to/".$image;
}
$description="";
?>
<rss version="2.0">
...
$videos = explode('megavideo/?v=', $html);
unset($videos[0]);
...
foreach($videos as $video) {
  $t1=explode('"',$video);
  $id=$t1[0];
  $link="http://www.megavideo.com/?v=".$id;
  $link="http://127.0.0.1/cgi-bin/scripts/filme/php/link1.php?file=".urlencode($link);
  $title="www.megavideo.com";
  $name = preg_replace('/[^A-Za-z0-9_]/','_',$tit).".flv";
