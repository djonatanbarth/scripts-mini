...
$url = $sThisFile."?query=".($page-1).",".$id.",".urlencode($tit);
?>
...
$videos = explode('class="galleryitem" id="post-', $html);
...
foreach($videos as $video) {
    ...
    $t2=explode('Online',$t1[1]);
    $title=trim($t2[0]);

    $desc = str_between($video,"<p>","</p>");
    $desc = preg_replace("/(<\/?)(\w+)([^>]*>)/e"," ",$desc);
    $desc=trim($desc);
    
		$link = 'http://127.0.0.1/cgi-bin/scripts/filme/php/onlinefilmletoltes1.php?file='.$link.','.urlencode($title).",".urlencode($image);
